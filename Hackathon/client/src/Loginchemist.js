...
function Search(){
  ...
  let submit= async(e)=>{
      ...
      try{
        ...
        await axios.post("http://localhost:4000/api/v1/Loginchemist",{email,password})
        .then((res)=>{setData(res.data.success);localStorage.setItem("success",res.data.success)
        localStorage.setItem("email",res.data.email);setData(res.data.success);localStorage.setItem("Role",res.data.data)
